...
{
    internal static class PrecipiceUtility
    {
        // Precipice only regrows missing parts, clears permanent scars, and heals injuries. If a pawn has
        // none of those, it has nothing to gain from Precipice (it can't cure disease) - so putting them
    ...
    // When a player deliberately orders a pawn to take REAL Precipice but that pawn has nothing for it to
    ...
    public static class Patch_PrecipiceConfirm
    {
        ...
        public static bool Prefix(Pawn_JobTracker __instance, Job newJob, Pawn ___pawn)
        {
            ...
            if (drug == null || drug.def.defName != "ME_Compound_Precipice" || ingester == null) return true;
            if (!PrecipiceUtility.HasNothingToHeal(ingester)) return true;
            ...
            Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                "ME_PrecipiceNoNeedConfirm".Translate(ingester.LabelShortCap),
